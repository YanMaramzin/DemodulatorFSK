...
class DemodulatorFSK{
    ...
    std::vector<int> demodulate(std::vector<T>& in,double inputSampleRate,double f0, double f1,double baudRate,const double stopBitDuration)
    {
        ...
        summaI0.reserve(6000);
        summaI1.reserve(6000);
        summaQ0.reserve(6000);
        summaQ1.reserve(6000);
        ...
        //Нормировка сигнала к значению его средней мощности (ещё в процессе)
        //Расчет средней мощности
//        double P;
//        for(int i=0; i<in.size()/4;i++)
//        {
//            P+=pow(in[i],2)/(in.size()/4);
//        }

//        difference[0]=difference[0]/P;
//        for(int i=1; i<difference.size()-1;i++)
//        {
//            difference[i]=difference[i]*difference[i-1]/P;
//        }
